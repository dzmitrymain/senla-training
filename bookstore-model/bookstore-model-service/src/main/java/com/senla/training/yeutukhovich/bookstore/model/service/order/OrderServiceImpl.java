...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderDto createOrder(Long bookId, String customerData) {
        Book book = bookDao.findById(bookId)
                .orElseThrow(() -> {
                    log.warn(LoggerConstant.CREATE_ORDER_FAIL.getMessage(), bookId,
        ...
        if (!book.getAvailable()) {
            ...
            log.info(LoggerConstant.CREATE_REQUEST_SUCCESS.getMessage(), requestId, bookId);
        }
        Order order = new Order(book, customerData);
        orderDao.add(order);
        log.info(LoggerConstant.CREATE_ORDER_SUCCESS.getMessage(), order.getId(), bookId);
    ...
    public OrderDto updateState(Long orderId, OrderDto orderDto) {
        if (!orderId.equals(orderDto.getId())) {
            log.warn(LoggerConstant.UPDATE_ORDER_FAIL.getMessage(), orderId,
    ...
    public List<OrderDto> findSortedAllOrders(String sortParam) {
        ...
        switch (sortParam) {
            case "COMPLETION":
                ...
                log.info(LoggerConstant.FIND_ALL_ORDERS_SORTED_BY_COMPLETION_DATE.getMessage());
                break;
            case "PRICE":
                result = orderDao.findSortedAllOrdersByPrice();
                log.info(LoggerConstant.FIND_ALL_ORDERS_SORTED_BY_PRICE.getMessage());
                break;
            case "STATE":
                result = orderDao.findSortedAllOrdersByState();
                log.info(LoggerConstant.FIND_ALL_ORDERS_SORTED_BY_STATE.getMessage());
                break;
            default:
    ...
    public List<OrderDto> findCompletedOrdersBetweenDates(Date startDate, Date endDate) {
        log.info(LoggerConstant.FIND_COMPLETED_ORDERS_BETWEEN_DATES.getMessage(),
    ...
    public BigDecimal calculateProfitBetweenDates(Date startDate, Date endDate) {
        log.info(LoggerConstant.SHOW_PROFIT_BETWEEN_DATES.getMessage(),
                DateConverter.formatDate(startDate, DateConverter.DAY_DATE_FORMAT),
                DateConverter.formatDate(endDate, DateConverter.DAY_DATE_FORMAT));
    ...
    public Long calculateCompletedOrdersNumberBetweenDates(Date startDate, Date endDate) {
        log.info(LoggerConstant.SHOW_COMPLETE_ORDERS_NUMBER_BETWEEN_DATES.getMessage(),
                DateConverter.formatDate(startDate, DateConverter.DAY_DATE_FORMAT),
                DateConverter.formatDate(endDate, DateConverter.DAY_DATE_FORMAT));
    ...
    public OrderDetailsDto showOrderDetails(Long orderId) {
        Order order = orderDao.findById(orderId)
                .orElseThrow(() -> {
                    log.warn(LoggerConstant.SHOW_ORDER_DETAILS_FAIL.getMessage(), orderId,
        ...
        log.info(LoggerConstant.SHOW_ORDER_DETAILS_SUCCESS.getMessage(), orderId);
    ...
    public List<OrderDto> exportAllOrders(String fileName) {
        ...
        log.info(LoggerConstant.EXPORT_ALL_ORDERS.getMessage(), fileName);
    ...
    public OrderDto exportOrder(Long id, String fileName) {
        ...
        Order order = orderDao.findById(id).orElseThrow(() -> {
            log.warn(LoggerConstant.EXPORT_ORDER_FAIL.getMessage(), id,
                    MessageConstant.ORDER_NOT_EXIST.getMessage());
            return new BusinessException(MessageConstant.ORDER_NOT_EXIST.getMessage(), HttpStatus.NOT_FOUND);
        });
        List<String> orderStrings = entityCsvConverter.convertOrders(List.of(order));
        FileDataWriter.writeData(path, orderStrings);
        log.info(LoggerConstant.EXPORT_ORDER_SUCCESS.getMessage(), id, fileName);
        return orderMapper.map(order);
    }
    ...
    public List<OrderDto> importOrders(String fileName) {
        ...
        for (Order importedOrder : importedOrders) {
            Book dependentBook = bookDao.findById(importedOrder.getBook().getId())
                    .orElseThrow(() -> {
                        log.warn(LoggerConstant.IMPORT_ORDERS_FAIL.getMessage(),
        ...
        log.info(LoggerConstant.IMPORT_ORDERS_SUCCESS.getMessage(), fileName);
    ...
    private OrderDto cancelOrder(Long orderId) {
        Order order = orderDao.findById(orderId)
                .orElseThrow(() -> {
                    log.warn(LoggerConstant.CANCEL_ORDER_FAIL.getMessage(), orderId,
                            MessageConstant.ORDER_NOT_EXIST.getMessage());
                    return new BusinessException(MessageConstant.ORDER_NOT_EXIST.getMessage(), HttpStatus.NOT_FOUND);
                });
        ...
        log.info(LoggerConstant.CANCEL_ORDER_SUCCESS.getMessage(), orderId);
        return orderMapper.map(order);
    }
    ...
    private OrderDto completeOrder(Long orderId) {
        Order order = orderDao.findById(orderId)
                .orElseThrow(() -> {
                    log.warn(LoggerConstant.COMPLETE_ORDER_FAIL.getMessage(), orderId,
                            MessageConstant.ORDER_NOT_EXIST.getMessage());
                    return new BusinessException(MessageConstant.ORDER_NOT_EXIST.getMessage(), HttpStatus.NOT_FOUND);
                });
        ...
        order.setCompletionDate(new Date());
        orderDao.update(order);
        log.info(LoggerConstant.COMPLETE_ORDER_SUCCESS.getMessage(), orderId);
        return orderMapper.map(order);
    }
